...
//Se crean las listas de estudiantes, tutores y proyectos
        ...
        while (( $data = fgetcsv ( $fp , 2048, ";","'" )) !== false ) { // Mientras hay líneas que leer...
            ...
            for($k=0;$k<12;$k++) {
                ...
                if (!(strlen($carrera)<=4)) {
                    ...
                    else $_ERRORES[] = 'Error de formato en el campo "Carrera", por favor revise la linea '.$linea_actual;
                }
...
//                if (!empty($_ERRORES)) {
//                    $_WARNING[] = 'El problema fue encontrado en la l&iacute;nea n&uacute;mero "'.$linea_actual.'".';
                    //break;
//                }
        ...
        if(empty($_WARNING) AND empty($_ERRORES)){
                ...
                if (numResultados($resultado)>0){
                    ...
                    $sql_temp="UPDATE usuario_estudiante SET ".
                        " usbid_usuario='$carnet', ".//usbid
                        " carnet='$carnet', ".//carnet
                        " cohorte='$carnet', ".//cohorte
                        " carrera='$carrera', ".//carrera
                        " estudiante_sede='', ".//estudiante sede
                        " email_sec='$email', ".//email_sec
                        " telf_hab='$tlf_hab', ". //tlf_hab
                        " telf_cel='$tlf_cel', ". //celular
                        " direccion='', ". //direccion
                        " sexo='$sexo' ".//sexo		
                        " WHERE usbid_usuario='$carnet'";
                        if ($modo_depuracion) echo "$sql_temp<br>"; 
                        else $resultado=ejecutarConsulta($sql_temp, $conexion);
                } else 
                ...
                if (!(numResultados($resultado)>0)){        
                    ...
                    if ($horas_acumuladas_est<120){
                        ...
                        if ($horas_cumplidas!=0)//En caso de que el estudiante no 
                            {$sql_temp.="DATE_ADD('$fecha_ins', INTERVAL 91 DAY),";}//fecha_fin_real
                        else
                            {$sql_temp.="'',";}
